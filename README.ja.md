...
**[▶ ブラウザで遊ぶ](https://mikami2026.github.io/3D_Tetris/)** — PC専用。キーボードが必要です。
...
- WebGL 2 対応のデスクトップブラウザ（現行の Chrome / Edge / Firefox / Safari）
- **キーボード。** 3軸の回転に6キー必要なため、スマホ・タブレットでは遊べません
- 目安として 1024×640 以上のウィンドウ

キーは刻印ではなく物理的な位置で判定しているので、配列が違っても `WASD` は同じ場所にあります。
AZERTY 配列では説明パネルの表記と刻印が一致しません。
